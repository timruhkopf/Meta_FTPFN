...
def get_batch(
    ...
    **kwargs,
    ...
    - where we sample a new bnn as task and interpolate between the outputs between both tasks to
    create a new task
    ...
    all_configs = np.random.uniform(
        size=(seq_len, int(batch_size*(1-share_unrelated_tasks)),  num_params)
    ...
    target_task_outputs = []
    for i in range(1, int(batch_size*(1-share_unrelated_tasks))):
    ...
    relatedness = np.random.beta(0.1, 20, size=(int(batch_size * (1-share_unrelated_tasks)),))
    related_task_curves = []
    for i in range(1, int(batch_size * (1 - share_unrelated_tasks))):
        # sample a new task

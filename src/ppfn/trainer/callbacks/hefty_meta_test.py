...
class HeftyContextEvaluationCallback(AbstractCallback):
    ...
    def on_train_end(self, **kwargs):
        ...
        if (
            self.dataset.target_first
        ):  # to change the way the batch and padding are parsed
        ...
        for ctx_size in self.context_levels:
            ...
            print(
                f"Hefty Eval: Target Context Size = {actual_ctx} | Eval Pos = {self.single_eval_pos}"
            )
    ...
    def _evaluate_at_context_level(self, visible_ctx: int):
        ...
        assert visible_ctx <= self.single_eval_pos, (
            "Visible context cannot exceed the single_eval_pos boundary."
        )
        assert visible_ctx > 0, (
            "Visible context must be greater than 0 for evaluation, otherwise padding will cause nan in attention!"
        )
        ...
        with torch.no_grad():
            for i, batch in enumerate(loader):
                ...
                _, step_metrics = self.trainer._forward_pass(
                    batch, self.single_eval_pos, src_key_padding_mask=pad_mask
                )

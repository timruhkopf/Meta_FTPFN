...
class UnifiedPFNLayer(nn.Module):
    ...
    def forward(self, x: Tensor, eval_pos: int, pad_mask: Optional[Tensor] = None) -> Tensor:
        ...
        # Slicing the mask ensures queries do not attend to padded context tokens
...
class PFNStack(nn.Module):
    ...
    def forward(self, context: Tensor, queries: Optional[Tensor] = None, pad_mask: Optional[Tensor] = None):
        """
        Expects context shape: (Time_C, Batch, Dim).
        Fails safely if context is guaranteed to have Time_C > 0.
        """
        eval_pos = context.shape[0]

        if queries is not None and queries.shape[0] > 0:
            combined = torch.cat([context, queries], dim=0)
        else:
            combined = context
